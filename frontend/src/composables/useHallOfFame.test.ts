...
describe('useHallOfFame', () => {
    const TWO_STAGE_PRESET = [
        { name: 'groups', label: 'Group Stage', groups: ['A'], racesRequired: 5, teamsAdvancingPerGroup: 0 },
    ...
    const makeTeam = (id: string, name: string, captainId: string, memberIds: string[] = []): Team => ({
        ...
        stagePoints: { groups: 0, finals: 0 },
        stageGroups: { groups: 'A', finals: 'A' },
        qualifiedStages: ['groups', 'finals'],
    });

    const makeRace = (id: string, placements: Record<string, number>, stage: string = 'groups', raceNumber: number = 1): Race => ({
    ...
    it('calculates Comeback Kings correctly', () => {
        const team1: Team = {
            ...makeTeam('t1', 'Team 1', 'p1'),
            stagePoints: { groups: 0, finals: 51 },
        };
        const team2: Team = {
            ...makeTeam('t2', 'Team 2', 'p2'),
            stagePoints: { groups: 0, finals: 50 },
        };
        const tournament: Tournament = {
            ...baseTournament,
            teams: [team1, team2],
