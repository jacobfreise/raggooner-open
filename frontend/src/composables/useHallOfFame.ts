...
const categories: FameCategory[] = [
    ...
    },
    {
        ...
        calculate: (t: Tournament) => {
            const playerValues = Object.values(t.players);
            const maxPts = Math.max(...playerValues.map(p => p.totalPoints || 0));
    ...
    },
    {
        ...
        calculate: (t: Tournament) => {
            ...
            const performances = Object.values(t.players).map(p => {
                const groupPlaces = Object.values(t.races)
                    .filter(r => r.stage === 'groups' && r.placements[p.id] !== undefined)
                    .map(r => r.placements[p.id]!);
    ...
    },
    {
        ...
        calculate: (t: Tournament) => {
            ...
            const variances = Object.values(t.players).map(p => {
    ...
    },
    {
        ...
        calculate: (t: Tournament) => {
            ...
            const yoyos = Object.values(t.players).map(p => {
    ...
    },
    {
        ...
        calculate: (t: Tournament) => {
            ...
            t.teams.forEach(team => {
                ...
                const members = [team.captainId, ...team.memberIds]
                    .map(id => t.players[id])
...
const processCategoryResults = (
    ...
    switch (tieHandling.type) {
        ...
        case 'no-winner-on-tie':
            return [];
        case 'tiebreaker':
            const sorted = [...rawResults].sort((a, b) => tieHandling.fn(a, b, tournament));
            const topScore = tieHandling.fn(sorted[0]!, sorted[0]!, tournament);
            const winners = sorted.filter(r => tieHandling.fn(r, sorted[0]!, tournament) === topScore);
            return winners.slice(0, 1);
    }
};

export function useHallOfFame(tournament: Ref<Tournament>) {
    const activeStats = computed(() => {
        if (!tournament.value) return [];
        return categories
            .map(cat => {
                const rawResults = cat.calculate(tournament.value);
                const processedResults = processCategoryResults(cat, rawResults, tournament.value);
