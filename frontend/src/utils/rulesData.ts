...
export interface FormatRules {
    unique: TournamentRule[];
    general: TournamentRule[];
}

const GENERAL_RULES: TournamentRule[] = [
...
const FORMAT_UNIQUE_RULES: Partial<Record<string, TournamentRule[]>> & { 'uma-ban': TournamentRule[] } = {
    'uma-draft': [
...
export function getTournamentRules(format: string | undefined): FormatRules {
    const unique = (format ? FORMAT_UNIQUE_RULES[format] : null) ?? FORMAT_UNIQUE_RULES['uma-ban'];
    return { unique, general: GENERAL_RULES };
}
